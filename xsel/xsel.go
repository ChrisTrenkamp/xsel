...
var xpath xsel.Grammar
var variableBindings = make(map[xsel.XmlName]xsel.Result)
...
func main() {
	...
	xpathTest, err := xsel.BuildExpr(*xpathExpr)
	...
	for name, value := range variableDeclarations {
		qName, err := xsel.GetQName(name, namespaces)
		...
		variableBindings[qName] = xsel.String(value)
	}
...
func runXpathOnStdin() {
	...
	cursor, err := createCursor(os.Stdin, *fileType)
...
func createCursor(in io.Reader, parseType string) (xsel.Cursor, error) {
	switch parseType {
	case "xml":
		return xsel.ReadXml(in, buildXmlParserSettings)
	case "html":
		return xsel.ReadHtml(in)
	case "json":
		return xsel.ReadJson(in)
	}

	return nil, fmt.Errorf("unsupported file type")
}

func executeXpath(cursor xsel.Cursor, path string) {
	result, err := xsel.Exec(cursor, &xpath, buildContextSettings)
	...
	nodeSet, isNodeSet := result.(xsel.NodeSet)
	...
	if isNodeSet && *asXml {
		...
		for _, node := range nodeSet {
			writeResult(&buffer, path, xsel.NodeSet{node})
		}
	} else {
...
func buildContextSettings(c *xsel.ContextSettings) {
	c.NamespaceDecls = namespaces
	c.Variables = variableBindings
}

func writeResult(buffer *bytes.Buffer, path string, result xsel.Result) {
...
func writeXmlResult(buffer *bytes.Buffer, path string, result xsel.NodeSet) {
...
func encodeCursorToXml(encoder *xml.Encoder, cursor xsel.Cursor) error {
...
func writeAttributeAsProcInst(encoder *xml.Encoder, attr xsel.Attribute) error {
	target := "attribute:"
...
func writeNamespaceAsProcInst(encoder *xml.Encoder, ns xsel.Namespace) error {
...
func writeElementToken(encoder *xml.Encoder, elem xsel.Cursor) error {
	n := elem.Node().(xsel.Element)
	t := xml.StartElement{
		Name: xml.Name{
			Space: n.Space(),
			Local: n.Local(),
...
func writeEndElement(encoder *xml.Encoder, elem xsel.Cursor) error {
	n := elem.Node().(xsel.Element)
	t := xml.EndElement{
...
func createXmlName(n xsel.NamedNode) xml.Name {
	return xml.Name{
		Space: n.Space(),
		Local: n.Local(),
	}
